...
- **vitest 4.1.11 -> 5.0.0 and `@vitest/coverage-v8` 4.1.11 -> 5.0.0 (majors), plus
  ...
  Worth recording because **this repo was the exception**: the same vitest 5 pair is BLOCKED
  in `deepthinking-mcp` (#308/#309) and `memory-mcp` (#172/#174), where the coverage job dies
  with `TypeError: Expected string coverage payload, received object` -- the v8 coverage
  payload changed shape in 5. math-mcp takes it cleanly because it runs no equivalent
  coverage job, which is what pins the blame on the coverage integration rather than on
  vitest itself.

  All three PRs first went red on plumbing, not on the bumps: Dependabot edits `package.json`
  and never `bun.lock`, while CI runs `bun install --frozen-lockfile`. The lock was
  regenerated by hand on each branch -- the second time this month a whole Dependabot backlog
  needed that step, so it is now filed as an automation gap rather than a chore.

- **Dev-dependency bump, with `bun.lock` regenerated by hand** (`@types/node` 26.4.0 -> 26.4.1,
  `eslint-plugin-jsdoc` 64.2.1 -> 64.3.4). Dependabot updates `package.json` only -- it cannot
  write `bun.lock` -- and CI runs `bun install --frozen-lockfile`, so the PR failed on every leg
  with `error: lockfile had changes, but lockfile is frozen`. Regenerated with `bun install`
  (4-line diff, exactly the two bumped versions); frozen install then succeeds, 12/12 integration
  tests pass, lint clean.
  **This is structural, not a one-off:** every Dependabot PR against this repo will fail the same
  way until something in the pipeline regenerates the lockfile. Note the side effect -- a human
  push takes the branch out of Dependabot's hands, so its `automerge` check goes SKIPPED.
