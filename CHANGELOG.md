...
- **Auto-merged Dependabot commits could land on the default branch with no CI run.** The
  ...
  that token's blast radius to close a telemetry hole rather than a gate hole. (4d8f2d3)
...
- **Added `docs/architecture/`** — generated by `repo_map.py map` (the `skills` repo's
  architecture-docs tooling): `dependency-graph.json`, `file-inventory.json`,
  `duplicate-symbols.json`, `unused-analysis.json`, plus authored `OVERVIEW.md` and
  `ARCHITECTURE.md` whose numeric claims carry Verification blocks checked by
  `repo_map.py check` (proven both ways: clean → exit 0, a flipped number → exit 1).
  Findings recorded in the docs, all hand-verified: zero circular dependencies; real
  source is 3,777 LOC (the 129k repo total is the committed 118k-line
  `bundle/index.mjs`); **four exported telemetry symbols are genuinely unused**
  (`activeConnections`, `mcpRequests`, `rateLimitQueue`, `getTelemetryServer` — each
  occurs only at its declaration; note the prom-client self-registration caveat before
  deleting); **`src/types.ts` is unimported by anything** despite its docstring's
  cross-layer claim; the other six "orphans" are verified-live direct-execution shapes
  (committed bundle, npm-script drivers).
