/**
 * @file matrix-minimal.cjs
 * @description Minimal JavaScript bindings for WASM matrix operations
 * Manages memory manually without requiring AssemblyScript runtime
 */
  ...
  const wasmModule = await WebAssembly.instantiate(wasmBuffer, {
    env: {
      ...
      abort: (msg, file, line, column) => {
        console.error(`Abort at ${file}:${line}:${column}`);
...
function allocateF64Array(data) {
  ...
  if (pagesNeeded > wasmMemory.buffer.byteLength / 65536) {
    wasmMemory.grow(pagesNeeded - (wasmMemory.buffer.byteLength / 65536));
  }
